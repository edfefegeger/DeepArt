document.getElementById("submitButton").addEventListener("click", async (event) => {
    ...
    try {
        const response = await fetch("https://localhost:8080/generate-video", {
            method: "POST",
            body: formData,
        });
        ...
        do {
            ...
            const statusResponse = await fetch(`https://localhost:8080/status/${task_id}`);
        ...
        const videoResponse = await fetch(`https://localhost:8080/get-video/${task_id}`);
        if (!videoResponse.ok) {
