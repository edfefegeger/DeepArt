document.getElementById("submitButton").addEventListener("click", async (event) => {
    ...
    try {
        const response = await fetch("https://35.214.184.4:8070/generate-video", {
            method: "POST",
            body: formData,
        });
        ...
        do {
            ...
            const statusResponse = await fetch(`https://35.214.184.4:8070/status/${task_id}`);
        ...
        const videoResponse = await fetch(`https://35.214.184.4:8070/get-video/${task_id}`);
        if (!videoResponse.ok) {
